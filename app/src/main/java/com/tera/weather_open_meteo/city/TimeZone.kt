...
fun Address.timezone(context: Context): String {
    // Источник: https://en.wikipedia.org/wiki/List_of_tz_database_time_zones
    val nonLocalizedAddress: Address by lazy(@Suppress("DEPRECATION")
    @Throws(IOException::class)
    fun(): Address {
    ...
    when (this.countryCode) {
        ...
        "AU" -> {
            ...
            when (nonLocalizedAddress.adminArea) {
                ...
                "New South Wales", "Australian Capital Territory", "Jervis Bay Territory" ->
                    return if (this.longitude < 141.9 && this.latitude > -32.2 && this.latitude < -31.6) "Australia/Broken_Hill"
        ...
        ) "Pacific/Bougainville" else "Pacific/Port_Moresby"
    ...
    // Если мы не можем найти страну, берем часовой пояс, в котором она находится, исходя из долготы.
    val utcOffset = (this.longitude / 15.0).roundToInt()
    // Здесь стоит знак минус, потому что знак часовых поясов Etc/UTC... инвертирован.
    return if (utcOffset < 0) "Etc/GMT+" + -utcOffset
